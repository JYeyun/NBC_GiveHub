        ...
        val dummyPostList = arrayListOf(
            ...
            MainPageItem(
                ...
                "코틀린에서 배열을 다루기 위한 다양한 유형들이 있다.\n\n" +
                        "크게 두 가지로 구분하자면, 가변형 (배열의 값을 바꿀 수 있음)과 불변형(가변형 반대)가 있다.\n\n" +
                        "불변형이라고 아예 리스트를 건드릴 수 없는 것은 아니다.\n" +
            ...
            MainPageItem(
                ...
                "onCreate() 메서드에서는 액티비티의 일회성 초기화를 실행해야 하는데, onCreate() 메서드는 액티비티가 초기화된 직후(OS에서 메모리에 새로운 액티비티 객체를 만들 때) 한 번 호출이 된다.onCreate()가 실행되면 활동이 생성되었다고 간주된다.\n\nonCreate() 메서드를 재정의할 때, 상위클래스 구현을 호출하여 활동 생성을 완료해야하는데, 이를 위해 액티비티 안에서 super.onCreate()를 즉시 호출해야한다.\n\n 이 사항은 다른 수명 주기 콜백 메서드의 경우에도 마찬가지로 적용이 필요하다.\n\n" +
                        "onStart() 메서드는 onCreate() 직후에 호출된다.onStart()가 실행되면 액티비티가 화면에 표시되며, onCreate()와 달리 시스템에서 활동의 수명 주기 동안 여러 번 호출할 수 있다.\n\nonStart()는 onStop() 메서드와 상응하여 페어링된다. 사용자가 앱을 시작한 후 기기 홈 화면으로 돌아오면 액티비티가 중지되고, 더 이상 화면에 표시되지 않게 된다.\n\n" +
                        "onPause()가 호출되면 더이상 앱에 포커스를 맞추고 있지 않게 되고, onStop() 이후에는 앱이 화면에 표시되지 않게된다.\n\n" +
                        "onRestart(), onStart()로 시작한 후 onResume()으로 앱이 재개된다.\n\n" +
                        "참고로 onRestart()는 액티비티가 이미 만들어진 상태에서 onStop()이 호출될 때 Created 상태로 전환된다.\n이때 Destroyed로 진입하지 않고 Started로 되돌아 가는 경우에만 시스템에서 호출한다.\n" +
                        "즉, onRestart()는 액티비티가 처음으로 시작되지 않은 경우에만(앱을 백그라운드에서 포그라운드로 다시 시작하는 경우에만) 호출하려는 코드를 배치하는 위치이다.",
                24,
                false
            ),
            MainPageItem(
            ...
            MainPageItem(
                ...
                "Activity Stack이 뭘까?",
                "Activity Stack 관리 : AndroidManifest의 LaunchMode의 옵션으로 관리한다.\n" +
